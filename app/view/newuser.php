	...
	class NewUser{
		public static function make($newuser=NULL){
			...
			if($newuser == NULL){
				...
				echo $twig->render('sign_up.html', array('title' => $title,'form_token' => $form_token, 'css_url' => '/css/authenticate.min.css'));
			}
			elseif($newuser->status==0){
				...
				$_SESSION['form_token'] = $form_token;
				$title = "Register | Organize";
				echo $twig->render('sign_up.html', array('title' => $title,'form_token' => $form_token, 'css_url' => '/css/authenticate.min.css', 'message' => $message));
			}
